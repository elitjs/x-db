...
|---|---:|---:|---:|---:|---:|---:|
| **x-db (sync)** | 1,007,419* | 5,147 | 1,004,859* | 982,578* | **538** | **338** |
| **x-db (nosync)** | 7,225 | 4,073 | 7,029 | 11,300 | **420** | **327** |
| SQLite (WAL+NORMAL) | 20,281 | 1,151 | 17,181 | 17,464 | 2,711 | 2,194 |
| redb (txn/op) | 1,158,275* | 3,653 | 1,190,617* | 1,214,143* | 417 | 397 |
| HashMap (RAM) | 276 | — | 357 | 225 | 214 | — |

`*` = fsync ทุก operation (durable จริง) — ต้นทุนถูกครอบด้วย disk sync (~1ms บนเครื่องนี้)

**อ่านผลอย่างเป็นธรรม:**
- **อ่าน**: x-db เร็วสุด — เจอ ~540-540ns (5 เท่าของ SQLite), ไม่เจอ ~330ns (6.5 เท่า)
- **เขียน durable จริง** (sync ทุก op): x-db ≈ redb (~1ms ถูกจำกัดโดย fsync ของดิสก์)
- **เขียนแบบ nosync**: x-db 7µs/op — เร็วกว่า SQLite(WAL) ~3 เท่า (แต่ durability ต่างชั้นกัน)
- **batch 1000 keys**: SQLite เร็ยสุด 1.2µs/key (WAL commit ไม่ fsync) — x-db 5µs/key (fsync ทุก batch)
- สรุป: งาน **อ่านเยอะ** x-db ชนะชัดเจน / งาน**เขียนถี่มาก ๆ** SQLite WAL ยังได้เปรียบด้าน raw throughput
